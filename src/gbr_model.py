...
def main(config_path: str) -> None:
    ...
    pred_path = write_oof_predictions(oof_df, cfg['paths']['output_dir'], 'gbr')
    metrics_path = write_cv_metrics(per_fold_metrics, cfg['paths']['output_dir'], 'gbr')
    summary_path = write_cv_summary(oof_df['rice_yield_pred'].to_numpy(), cfg['paths']['output_dir'], 'gbr')
    ...
    if cv_cfg['refit_full'] and cfg.get('output', {}).get('save_model', True):
        ...
        model_path = save_model(final_model, cfg['paths']['models_dir'], 'gbr')
        print(f"Saved model to: {model_path}")
        if cfg.get('output', {}).get('create_sample', True):
            sp_pred, sp_sum = run_synthetic_self_check(pre, cfg, final_model, 'gbr')
