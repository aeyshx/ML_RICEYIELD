...
def main(config_path: str) -> None:
    ...
    use_tuning = rf_config.get('use_hyperparameter_tuning', False)
    ...
    if use_tuning:
        ...
        param_grid = {
            'n_estimators': rf_config.get('n_estimators', [100, 200, 300]),
            'max_depth': rf_config.get('max_depth', [None, 5, 10]),
            'min_samples_leaf': rf_config.get('min_samples_leaf', [1, 2, 5]),
            'max_features': rf_config.get('max_features', ['sqrt', 'log2'])
        }
        ...
        model = RandomForestRegressor(
            n_estimators=rf_config.get('default_n_estimators', 300),
            max_depth=rf_config.get('default_max_depth', None),
            min_samples_split=rf_config.get('default_min_samples_split', 2),
            min_samples_leaf=rf_config.get('default_min_samples_leaf', 1),
            max_features=rf_config.get('default_max_features', 'sqrt'),
            bootstrap=rf_config.get('bootstrap', True),
            n_jobs=rf_config.get('n_jobs', -1),
            oob_score=rf_config.get('oob_score', False),
            random_state=cfg.get('random_state', 42)
        ...
        print(f"Training RF with config: n_estimators={rf_config.get('default_n_estimators', 300)}, "
              f"max_depth={rf_config.get('default_max_depth', None)}, "
              f"max_features={rf_config.get('default_max_features', 'sqrt')}")
    ...
    if should_save:
        ...
        model_path = save_model(model_to_save, cfg['paths']['models_dir'], 'rf')
    ...
    metrics_path = write_metrics_and_diagnostics(ds.y_test, preds, ds.test_index, cfg['paths']['output_dir'], 'rf')
